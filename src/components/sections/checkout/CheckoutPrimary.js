...
const CheckoutPrimary = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    if (formData.phone && !phoneRegex.test(formData.phone.trim())) {
      setFieldErrors({
        ...fieldErrors,
        phone: t(
          'Phone must start with 966 followed by 9 digits (e.g., 966512345678)'
        ),
      });
      setLoading(false);
    ...
    const formattedPayload = {
      address: {
        ...
        street: [
          ...
          .filter(Boolean)
          .join(', '), // Concatenate and remove empty parts
    ...
    try {
      ...
      // Place the order
      const { data } = await axiosInstance.post(endpoint, formattedPayload, {
        headers,
      });
  ...
  useEffect(() => {
    if (userData) {
      ...
      const addressParts =
        userData.address?.[0]?.street?.split(',').map((part) => part.trim()) ||
        [];
  ...
  useEffect(() => {
    // Track checkout initiation when page loads with products
    if (isProducts && products.items.length > 0) {
      const totalAmount = products.totalPrice || cartTotal;

      // TIKTOK InitiateCheckout
      tiktokEvents.trackInitiateCheckout(products.items, totalAmount);

      // FACEBOOK InitiateCheckout (optional - for consistency)
      if (window.fbq) {
        const contentIds = products.items.map((item) => item.product._id);
        window.fbq('track', 'InitiateCheckout', {
          value: totalAmount,
          currency: 'SAR',
          content_ids: contentIds,
          content_type: 'product',
          num_items: products.items.length,
        });
      }

      // SNAPCHAT InitiateCheckout (optional)
      if (window.snaptr) {
        window.snaptr('track', 'START_CHECKOUT', {
          price: totalAmount,
          currency: 'SAR',
          item_ids: products.items.map((item) => item.product._id),
          number_items: products.items.reduce(
      ...
      console.log(
        'Checkout initiated tracked for:',
                ...
                <div className="ltn__checkout-single-content ltn__returning-customer-wrap">
                  ...
                  >
                    <div className="ltn_coupon-code-form ltn__form-box">
                      <p>{t('Please login your account.')}</p>
                          ...
                          <div className="col-md-6">
                            ...
                            <div className="input-item">
                              <input
                                ...
                                required
                                className={
                                  fieldErrors.houseNumber ? 'input-error' : ''
                                }
                              />
                            </div>
                          </div>
                          <div className="col-md-6">
                            ...
                            <div className="input-item">
                              <input
                                ...
                                required
                                className={
                                  fieldErrors.district ? 'input-error' : ''
                                }
                              />
                            </div>
                          </div>
                          ...
                          <div className="col-md-6">
                            ...
                            <div className="input-item">
                              <input
                                ...
                                required
                                className={
                                  fieldErrors.street ? 'input-error' : ''
                                }
                              />
                            </div>
                          </div>
                          <div className="col-md-6">
                            ...
                            <div className="input-item">
                              <input
                                ...
                                required
                                className={
                                  fieldErrors.landmark ? 'input-error' : ''
                                }
                              />
                            </div>
                          </div>
                ...
                <table className="table">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>
                        <strong>
                          {Number(products.subTotal).toFixed(2)}{' '}
                          <svg
                            id="Layer_1"
                            data-name="Layer 1"
                    ...
                    {products.discount > 0 && (
                      <tr>
                        ...
                        <td>
                          <strong>
                            -{Number(products.discount).toFixed(2)}{' '}
                            <svg
                              id="Layer_1"
                              data-name="Layer 1"
                              xmlns="http://www.w3.org/2000/svg"
                    ...
                    {products.shippingFee > 0 && (
                      <tr>
                        <td>
                          <strong>{t('Shipping and Handling')}</strong>
                        </td>
                        <td>
                          <strong>
                            {Number(products.shippingFee).toFixed(2)}{' '}
                            <svg
                              id="Layer_1"
                              data-name="Layer 1"
                              xmlns="http://www.w3.org/2000/svg"
                    ...
                    {products.shippingDiscount > 0 && (
                      <tr>
                        <td>
                          <strong>{t('Shipping Discount')}</strong>
                        </td>
                        <td>
                          <strong>
                            -{Number(products.shippingDiscount).toFixed(2)}{' '}
                            <svg
                              id="Layer_1"
                              data-name="Layer 1"
                              xmlns="http://www.w3.org/2000/svg"
                    ...
                    <tr>
                      ...
                      <td>
                        <strong>
                          {/* {((products.totalPrice) *0.15).toFixed(2)}  */}0{' '}
                          <svg
                            id="Layer_1"
                            data-name="Layer 1"
                            xmlns="http://www.w3.org/2000/svg"
